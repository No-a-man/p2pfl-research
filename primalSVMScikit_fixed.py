...
for i in range(min(5, len(X_data))):
    print(f"Sample {i+1}:")
    print(f"  Image shape: {X_data[i].shape}")
    print(f"  Image min/max: {X_data[i].min():.3f} / {X_data[i].max():.3f}")
    print(f"  Original label: {y_data[i]} ({'Zero' if y_data[i] == 0 else 'Non-zero'})")
    print(f"  First 10 pixels: {X_data[i][:10]}")
    print(f"  Last 10 pixels: {X_data[i][-10:]}")
    print()

# Find and show specifically data labelled as 0 (zero digits)
zero_indices = np.where(y == 0)[0]
print(f"\n🔍 Data Labelled as 0 (Zero Digits) - Found {len(zero_indices)} samples:")
print(f"=" * 60)
for i, idx in enumerate(zero_indices[:5]):  # Show first 5 zero samples
    print(f"Zero Sample {i+1} (Index {idx}):")
    print(f"  Image shape: {X_data[idx].shape}")
    print(f"  Image min/max: {X_data[idx].min():.3f} / {X_data[idx].max():.3f}")
    print(f"  Original label: {y_data[idx]} (Zero)")
    print(f"  First 10 pixels: {X_data[idx][:10]}")
    print(f"  Last 10 pixels: {X_data[idx][-10:]}")
...
# Print 5 lines of test data for inspection
print(f"\n📋 Sample Test Data (first 5 samples):")
print(f"=" * 60)
for i in range(min(5, len(X_test))):
    print(f"Sample {i+1}:")
    print(f"  Image shape: {X_test[i].shape}")
    print(f"  Image min/max: {X_test[i].min():.3f} / {X_test[i].max():.3f}")
    print(f"  Label: {y_test[i]} ({'Zero' if y_test[i] == 1 else 'Non-zero'})")
    print(f"  First 10 pixels: {X_test[i][:10]}")
    print(f"  Last 10 pixels: {X_test[i][-10:]}")
    print()

# Find and show specifically test data labelled as 1 (zero digits in binary classification)
zero_test_indices = np.where(y_test == 1)[0]
print(f"\n🔍 Test Data Labelled as 1 (Zero Digits) - Found {len(zero_test_indices)} samples:")
print(f"=" * 60)
for i, idx in enumerate(zero_test_indices[:5]):  # Show first 5 zero test samples
    print(f"Zero Test Sample {i+1} (Index {idx}):")
    print(f"  Image shape: {X_test[idx].shape}")
    print(f"  Image min/max: {X_test[idx].min():.3f} / {X_test[idx].max():.3f}")
    print(f"  Binary label: {y_test[idx]} (Zero)")
    print(f"  First 10 pixels: {X_test[idx][:10]}")
    print(f"  Last 10 pixels: {X_test[idx][-10:]}")
    print(f"  Non-zero pixels count: {np.count_nonzero(X_test[idx])}")
    print(f"  Mean pixel value: {X_test[idx].mean():.3f}")
    print()
